...
**Phase 2 — DATA_MODEL Review 준비** (CEO Decisions 반영 초안 / Fable Gate 대기)
...
| ADR-006 | Raw GPS Location Metadata (CEO Decision #006, Pending Gate Review) | [docs/adr/ADR-006](docs/adr/ADR-006-raw-gps-location-metadata.md) |
| ADR-007 | Storage is Export (Pending Gate Review) | [docs/adr/ADR-007](docs/adr/ADR-007-storage-is-export.md) |

### Next Phase

Phase 2 — DATA_MODEL.md 확정 전 Gate Review

`DATA_MODEL.md` v0.3 Draft는 다음 CEO 결정을 반영한 Gate review 대상이다.

1. MVP Log Type은 `Message`, `Photo`만 지원한다.
2. Metadata는 추출만 하고 해석하지 않는다. AI/NLP 분류는 Future Work다.
3. VaultFolder는 사용자가 직접 담은 원본 큐레이션이며, Metadata 기반 탐색 결과는 재생성 가능한 View/Index다.
...
기록 방식: 1, 2, 3, 5는 `DATA_MODEL.md` v0.3 Draft 승인 범위에 묶고, 4는
ADR-007 Proposed, 6은 ADR-006 Proposed로 별도 심사한다.

### Next Gate

DATA_MODEL Review (Claude Code Fable Gate)
...
Chat / Photo (MVP) / Schedule / Place / Note ... (Future Work)
   ...
   Timeline          Log가 시간 순으로 그대로 이어진 기록 (MVP는 Event 없이 바로 여기로)
        ├── VaultFolder     사용자가 직접 담은 원본 큐레이션
        └── View/Index      Metadata 기반 탐색 결과 (재생성 가능)

Future: AI View        원본을 읽어 만드는 요약/추천/분류 제안 (원본은 절대 수정하지 않음)
```
...
- [docs/PRD.md](docs/PRD.md) — 제품 정의 (v0.2.4 Draft)
...
- [x] DATA_MODEL.md v0.3 Draft에 Auth / Device / Session 모델 반영
- [x] ADR-006 Proposed 초안 작성
- [x] ADR-007 Proposed 초안 작성
- [ ] Claude Code Fable DATA_MODEL Review
- [ ] CEO final approval after Fable report
