...
- VaultFolder Curation 제외: MVP는 Timeline, Search, Metadata Exploration까지만 지원
...
4. **Search is derived.** Metadata 기반 탐색 조건과 검색 결과는 재생성 가능한
   View/Index로 취급한다. MVP에는 사용자 큐레이션 원본 엔터티인 VaultFolder가
   없다.
5. **AI as Reader.** AI는 원본 데이터(Log, Media, Timeline)를 생성하거나
...
| `VaultFolder` | 사용자가 직접 만든 폴더와 그 안에 담은 Log 목록 | Future Work |
| `View` / `Index` | 시스템이 원본에서 만든 검색/탐색 파생 결과 | MVP 파생물, AI View는 Future Work |
...
## 7. Search and Metadata Exploration

MVP의 탐색은 Timeline, Deterministic Metadata, Search로 제공한다.

Metadata Exploration은 기간, 타입, GPS 존재 여부 같은 조건으로 Timeline과
Log를 탐색하는 재생성 가능한 View/Index다. 원본 Log, MediaRef, Timeline에
저장하지 않는다.

자동 Event 생성, AI 분류, VaultFolder Curation은 하지 않는다.

허용되는 MVP 탐색 기준:

- 기간
- Log Type (`Message`, `Photo`)
- 사용자(`actor_id`)
- 메시지에 직접 포함된 단어
- 사진 Metadata 존재 여부 또는 값(EXIF/GPS/해상도/파일 크기/해시)

`metadata_filter`나 조건 기반 검색 결과는 원본 엔터티 필드가 아니다.
저장된 검색, 스마트 폴더, AI 제안, 사용자가 직접 만든 VaultFolder가 나중에
도입되더라도 원본 Log/Timeline을 직접 변경하지 않고 `View` 또는 재생성 가능한
인덱스로만 제공한다.
...
GPS 존재 여부 같은 조건으로 Timeline을 필터링한 결과는 원본 엔터티가 아니라
재생성 가능한 View/Index다.
...
### 12.1 Future Work: VaultFolder

VaultFolder는 사용자가 직접 추억 폴더를 만들거나 AI가 큐레이션을 제안하는
기능이다. 도입 시점에는 다음 원칙을 다시 검토한다.

- 원본 사진과 Message Log는 Timeline 저장소에 한 번만 보존한다.
- VaultFolder는 원본을 복사하지 않고 `log_id` 참조나 재생성 가능한 View로
  표현한다.
- 보기 편한 별도 패키지가 필요하면 canonical storage가 아니라 generated
  Export View로 만든다.
...
- [x] VaultFolder Curation이 Future Work로 분리되어 있다.
