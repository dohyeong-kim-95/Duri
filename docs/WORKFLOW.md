...
- AI가 생성하는 모든 View(요약/추천/분류 제안 등)는 원본을 변경하지 않고,
  실패해도 Log/Timeline/Vault는 영향받지 않는다
...
Timeline + Vault 경험이 검증된 이후 다음이 추가될 수 있다
...
- AI as Reader: AI/자동화 로직은 Log/Timeline/Vault 원본을 절대 직접
  수정하지 않는다. 모든 산출물은 원본으로부터 재생성 가능한 View로 취급한다
