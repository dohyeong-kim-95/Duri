...
> [PRD v0.2.2 §4.3 Timeline First](./PRD.md#43-timeline-first)는 MVP에서
> Event를 생성하지 않는다고 명시한다. MVP는 Log를 Timeline에 직접 쌓고,
> 이후 Event Engine이 활용할 수 있도록 [Metadata](./DATA_MODEL.md#32-metadata-mvp-핵심-기능)를
> 축적하는 데 집중한다 ([PRD §6 Future Work](./PRD.md#6-mvp-goal)).
>
> 이 문서는 Timeline + Vault 경험이 검증된 이후 착수할 Event Engine의
> 설계 스케치를 미리 남겨두는 문서다. 엔티티 정의는
> [DATA_MODEL.md §7 Future Work: Event](./DATA_MODEL.md#7-future-work-event)를 따른다.
...
```
 ...
 New Log│  Ingest   │  Log 저장 완료 (source 무관: chat, upload, calendar_sync ...)
        ...
        │  & Vault   │ Vault 분류기 재실행 (신규/변경된 Event만)
        └───────────┘
```
...
| `time`         | `Log.created_at` |
| `geo`          | `Log.location` |
| `participants` | `Log.actor` (+ payload 내 인물 언급) |
...
## 8. Vault Suggestion (연계)

Vault는 태그가 아니라 사람이 관리하는 폴더다
([PRD §4.4](./PRD.md#44-vault), [DATA_MODEL.md §5](./DATA_MODEL.md#5-vault)).
Event Engine이 도입되더라도 Vault 폴더에 Log를 직접 넣는 것은 아니다.
대신 확정된 Event를 근거로 "이 Log들을 OO 폴더에 담는 게 어떤가요?" 같은
**제안(View)**만 만들고, 실제로 폴더에 담을지는 사용자가 결정한다
([PRD §4.5 AI as Reader](./PRD.md#45-ai-as-reader)). 상세 알고리즘은
추후 RFC에서 다룬다.
...
- Event Engine 착수 시점을 가늠할 기준(Timeline + Vault 경험이
